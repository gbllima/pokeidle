-- chunkname: @/modules/game_ruleviolation/ruleviolation.lua
...
function init()
	...
	reasonComboBox = ruleViolationWindow:getChildById("reasonCombo")
	actionComboBox = ruleViolationWindow:getChildById("actionCombo")
	reasonComboBox.onOptionChange = onSelectedReason
...
function hasWindowAccess()
	return reasonComboBox:getOptionsCount() > 0
end
...
function loadReasons()
	...
	for reason, actionFlags in pairs(actions) do
		...
		reasonComboBox:addOption(rvreasons[reason], data)
	end
...
function onSelectedReason(widget, text, data)
	actionComboBox:clear()

	for actionBaseFlag = 0, #rvactions do
		local actionFlagString = rvactions[actionBaseFlag]

		if bit32.band(data.actionFlags, math.pow(2, actionBaseFlag)) > 0 then
			actionComboBox:addOption(actionFlagString, actionBaseFlag)
		end
	end
end

function report()
	local reasonLabel = reasonComboBox:getCurrentOption()
	...
	local actionLabel = actionComboBox:getCurrentOption()
	...
	local reason = reasonLabel.data.reasonId
	local action = actionLabel.data
