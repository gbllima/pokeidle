-- chunkname: @/modules/game_minigame/photographer.lua
...
local function createTilePanel()
	panel = g_ui.createWidget("MiniGamePhotographerPanel", modules.game_interface.getMapPanel())

	local marginBottom = panel:getHeight() + OutfitSize * 2 - 12
	local marginRight = OutfitSize + panel:getHeight() / 2 - 15
...
local function bindKeyDown()
	...
	if isFail then
		...
		return scheduleEvent(function()
			modules.game_minigame.sendPhotographerGame(Actions.Action, Positions)
		end, 1350)
	end
...
local function onPhotographer(params)
...
function PhotographerGame.init()
	connect(PhotographerGame, {
...
function PhotographerGame.terminate()
	disconnect(PhotographerGame, {
		onPhotographer = onPhotographer
	})
	disconnect(g_game, {
		onGameEnd = parseEnd
