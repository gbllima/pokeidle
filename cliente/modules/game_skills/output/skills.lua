-- chunkname: @/modules/game_skills/skills.lua
...
function init()
	...
	ProtocolGame.registerExtendedJSONOpcode(ExtendsOpcodes.SkillBar, parseSkillBar)

	skillsButton = modules.client_topmenu.addRightButton("skillsButton", tr("Skills"), "/images/topbuttons/icon_profile_player", toggle, false, 1)

	skillsButton:setOn(false)

	skillsWindow = g_ui.loadUI("skills", modules.game_interface.getRightPanel())
	insigniasPanel = skillsWindow:recursiveGetChildById("skillId13")
...
function terminate()
	...
	ProtocolGame.unregisterExtendedJSONOpcode(ExtendsOpcodes.SkillBar)
...
function setSkillBadge(value)
	local badge = insigniasPanel[value]

	if badge then
		badge:setItemId(value)
	end
end
...
function refresh()
	...
	for i, v in pairs(InsigniasInfo) do
		local insignia = g_ui.createWidget("Insignias", insigniasPanel)
		local tmpItem = Item.create(v.badgeOff)

		tmpItem:setTooltip(v.tooltip)
		insignia:setItem(tmpItem)
		insignia:setId(v.badgeOn)
	end
	...
	if hasAdditionalSkills then
		...
		skillsWindow:setContentMaximumHeight(200)
	end
end
...
function offline()
	...
	insigniasPanel:destroyChildren()
end
...
function onStaminaChange(localPlayer, stamina)
	...
	local percent = math.floor(100 * stamina / 2520)
...
function parseSkillBar(protocol, opcode, json_data)
	local id = json_data.action
	local value = json_data.data

	signalcall(LocalPlayer.onExtraSkillChange, LocalPlayer, id, value)
end
