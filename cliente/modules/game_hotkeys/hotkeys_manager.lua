...
function init()
	hotkeysButton = modules.client_topmenu.addLeftButton("hotkeysButton", tr("Hotkeys") .. " (Ctrl+K)", "/images/topbuttons/hotkeys", toggle, false, 3)
...
function prepareKeyCombo(keyCombo, ticks)
	...
	if not hotKey or modules.game_console.isChatEnabled() and not g_keyboard.isSkipKey(keyCombo) or modules.game_minigame.isPlaying() then
		return
	end
