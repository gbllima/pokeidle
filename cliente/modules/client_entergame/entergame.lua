...
local function onCharacterList(protocol, characters, account, otui)
  ...
  if account and account.email then
    G.accountEmail = account.email
  else
  end

  if G.clientVersion == nil then
    G.clientVersion = 1098
  end

  if rememberPasswordBox:isChecked() then
    local accountEnc = g_crypt.encrypt(G.account)
    local passwordEnc = g_crypt.encrypt(G.password)
    g_settings.set('account', accountEnc)
    g_settings.set('password', passwordEnc)
  else
  ...
  local protoVersion = g_game.getClientProtocolVersion(G.clientVersion)
  g_game.setProtocolVersion(protoVersion)
...
if account and account.emailActive ~= nil and not account.emailActive then
  modules.game_accounts.showActivePanel()
else
...
function EnterGame.init()
	...
	serverSelector = enterGame.panel.serverSelector
	rememberPasswordBox = enterGame.panel.rememberPasswordBox

	if Servers ~= nil then
		serverSelector:setVisible(table.size(Servers) > 1)
		enterGame.panel.selecteServLabel:setVisible(table.size(Servers) > 1)
  ...
  local clientVersion = g_settings.get('client-version')
	...
	enterGame.panel.accountPasswordTextEdit:setText(password)
	enterGame.panel.accountNameTextEdit:setText(account)
...
function EnterGame.show()
	...
	enterGame:raise()
	enterGame:focus()
	enterGame.panel.accountNameTextEdit:focus()
end
...
function EnterGame.clearAccountFields()
	enterGame.panel.accountNameTextEdit:clearText()
	enterGame.panel.accountPasswordTextEdit:clearText()
	enterGame.panel.accountNameTextEdit:focus()
...
function EnterGame.onServerChange()
	...
	if Servers and Servers[server] ~= nil then
		serverHost = type(Servers[server]) == "table" and Servers[server][1] or Servers[server]
		G.host = serverHost
	end
end
...
function EnterGame.doLogin()
	...
	G.account = enterGame.panel.accountNameTextEdit:getText()
	G.password = enterGame.panel.accountPasswordTextEdit:getText()
	G.stayLogged = true
	G.server = serverSelector:getText():trim()
	G.host = serverHost
	G.clientVersion = tonumber(clientVersion)
	...
	if G.host:lower():find("http") ~= nil then
		...
		elseif #server_params >= 3 and tostring(tonumber(server_params[3])) == server_params[3] then
...
function EnterGame.doLoginHttp()
	...
	if Servers and Servers[server] ~= nil then
		if type(Servers[server]) == "table" then
			...
			HTTP.postJSON(G.host .. API.ACCOUNTS.AUTHENTICATION, data, onHTTPResult)
		end
	end
