...
commandEnv.slots = function()
  ...
  for slot, name in pairs(slotNames) do
    ...
    if item then
      print(string.format("%s: %s (ID: %d)", name, item:getName(), item:getId()))
    else
...
function init()
 ...
 -- commandTextEdit:setHeight(commandTextEdit.baseHeight)
  ...
  -- Adiciona botão do terminal para mobile
  if g_app.isMobile() then
    addEvent(function()
      if modules.client_topmenu then
        modules.client_topmenu.addRightButton('terminalButton', tr('Terminal'), '/images/topbuttons/options', toggle, 150)
      end
    end, 1000)
  end
