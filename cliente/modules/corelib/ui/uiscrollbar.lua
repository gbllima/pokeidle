...
local function updateSliderBackground(self)
	local sliderBackground = self:getChildById("sliderBackground")

	if sliderBackground == nil then
		return
	end

	local decrementButton = self:getChildById("decrementButton")
	local incrementButton = self:getChildById("incrementButton")
	local isVisibleDecrement = decrementButton:isVisible()
	local isVisibleIncrement = incrementButton:isVisible()

	if self.orientation == "vertical" then
		sliderBackground:setMarginTop(isVisibleDecrement and decrementButton:getHeight() or 0)
		sliderBackground:setMarginBottom(isVisibleIncrement and incrementButton:getHeight() or 0)
	else
		sliderBackground:setMarginLeft(isVisibleDecrement and decrementButton:getWidth() or 0)
		sliderBackground:setMarginRight(isVisibleIncrement and incrementButton:getWidth() or 0)
	end
end
...
local function calcValues(self)
	...
	if self.orientation == "vertical" then
		pxrange = self:getHeight() - (isVisibleDecrement and decrementButton:getHeight() - decrementButton:getMarginTop() - decrementButton:getMarginBottom() or 0) - (isVisibleIncrement and incrementButton:getHeight() - incrementButton:getMarginTop() - incrementButton:getMarginBottom() or 0)
		center = self:getY() + math.floor(self:getHeight() / 2)
	else
		pxrange = self:getWidth() - (isVisibleDecrement and decrementButton:getWidth() - decrementButton:getMarginLeft() - decrementButton:getMarginRight() or 0) - (isVisibleIncrement and incrementButton:getWidth() - incrementButton:getMarginLeft() - incrementButton:getMarginRight() or 0)
	...
	local px = math.max(proportion * pxrange, 15)
	...
	if range == 0 or self.value == self.minimum then
		if self.orientation == "vertical" then
			offset = -math.floor((self:getHeight() - px) / 2) + (isVisibleDecrement and decrementButton:getMarginRect().height or 0)
		else
			offset = -math.floor((self:getWidth() - px) / 2) + (isVisibleDecrement and decrementButton:getMarginRect().width or 0)
		end
	elseif range > 1 and self.value == self.maximum then
		if self.orientation == "vertical" then
			offset = math.ceil((self:getHeight() - px) / 2) - (isVisibleIncrement and incrementButton:getMarginRect().height or 0)
		else
			offset = math.ceil((self:getWidth() - px) / 2) - (isVisibleIncrement and incrementButton:getMarginRect().width or 0)
		end
	elseif range > 1 then
