-- chunkname: @/modules/corelib/ui/uiitemmessagebox.lua

if not UIWindow then
	dofile("uiwindow")
end

UIItemMessageBox = extends(UIWindow, "UIItemMessageBox")
UIItemMessageBox.create = nil

function UIItemMessageBox.display(title, message, item, buttons, onEnterCallback, onEscapeCallback)
	local messageBox = UIItemMessageBox.internalCreate()

	rootWidget:addChild(messageBox)
	messageBox:setStyle("MessageBoxWindow")
	messageBox:setText(title)

	local messageLabel = g_ui.createWidget("MessageBoxLabel", messageBox)

	messageLabel:setText(message)

	local itemWidget = g_ui.createWidget("Item", messageBox)

	itemWidget:setVirtual(true)
	itemWidget:setItem(item)
	itemWidget:setPhantom(true)
	itemWidget:addAnchor(AnchorHorizontalCenter, "parent", AnchorHorizontalCenter)
	itemWidget:addAnchor(AnchorVerticalCenter, "parent", AnchorVerticalCenter)

	local buttonsWidth = 0
	local buttonsHeight = 0
	local anchor = AnchorRight

	if buttons.anchor then
		anchor = buttons.anchor
	end

	local buttonHolder = g_ui.createWidget("ItemMessageBoxButtonHolder", messageBox)

	buttonHolder:addAnchor(anchor, "parent", anchor)

	for i = 1, #buttons do
		local button = messageBox:addButton(buttons[i].text, buttons[i].callback, buttons[i].color)

		if i == 1 then
			button:setMarginLeft(0)
			button:addAnchor(AnchorBottom, itemWidget:getId(), AnchorBottom)
			button:addAnchor(AnchorLeft, "parent", AnchorLeft)

			buttonsHeight = button:getHeight()
		else
			button:addAnchor(AnchorBottom, "prev", AnchorBottom)
			button:addAnchor(AnchorLeft, "prev", AnchorRight)
		end

		buttonsWidth = buttonsWidth + button:getWidth() + button:getMarginLeft()
	end

	buttonHolder:setWidth(buttonsWidth)
	buttonHolder:setHeight(buttonsHeight)

	if onEnterCallback then
		connect(messageBox, {
			onEnter = onEnterCallback
		})
	end

	if onEscapeCallback then
		connect(messageBox, {
			onEscape = onEscapeCallback
		})
	end

	messageBox:setWidth(math.max(messageLabel:getWidth(), messageBox:getTextSize().width, buttonHolder:getWidth()) + messageBox:getPaddingLeft() + messageBox:getPaddingRight())
	messageBox:setHeight(messageLabel:getHeight() + messageBox:getPaddingTop() + messageBox:getPaddingBottom() + buttonHolder:getHeight() + buttonHolder:getMarginTop() + itemWidget:getHeight() + 20)

	return messageBox
end

function displayItemInfoBox(title, message, item)
	local messageBox

	local function defaultCallback()
		messageBox:ok()
	end

	messageBox = UIItemMessageBox.display(title, message, item, {
		{
			color = "Blue",
			text = "Ok",
			callback = defaultCallback
		}
	}, defaultCallback, defaultCallback)

	return messageBox
end

function displayItemErrorBox(title, message, item)
	local messageBox

	local function defaultCallback()
		messageBox:ok()
	end

	messageBox = UIItemMessageBox.display(title, message, item, {
		{
			color = "Blue",
			text = "Ok",
			callback = defaultCallback
		}
	}, defaultCallback, defaultCallback)

	return messageBox
end

function displayItemCancelBox(title, messag, iteme)
	local messageBox

	local function defaultCallback()
		messageBox:cancel()
	end

	messageBox = UIItemMessageBox.display(title, message, item, {
		{
			color = "Red",
			text = "Cancel",
			callback = defaultCallback
		}
	}, defaultCallback, defaultCallback)

	return messageBox
end

function displayGeneralBox(title, message, item, buttons, onEnterCallback, onEscapeCallback)
	return UIItemMessageBox.display(title, message, item, buttons, onEnterCallback, onEscapeCallback)
end

function UIItemMessageBox:addButton(text, callback, color)
	local buttonHolder = self:getChildById("buttonHolder")
	local button = g_ui.createWidget("MessageBoxButton" .. color, buttonHolder)

	button:setText(text)
	connect(button, {
		onClick = callback
	})

	return button
end

function UIItemMessageBox:ok()
	signalcall(self.onOk, self)

	self.onOk = nil

	self:destroy()
end

function UIItemMessageBox:cancel()
	signalcall(self.onCancel, self)

	self.onCancel = nil

	self:destroy()
end
