-- chunkname: @/modules/corelib/ui/uitabbar.lua
...
function UITabBar:getButtonsPanel()
	return self.buttonsPanel
end

function UITabBar:addTabWidget(widget, panel, icon)
	if panel == nil then
		panel = g_ui.createWidget(self:getStyleName() .. "Panel")
	...
	panel.isTab = true
	widget.tabPanel = panel
	widget.tabBar = self

	widget:setId("tab")

	widget.onClick = onTabClick
	widget.onMouseRelease = onTabMouseRelease

	function widget.onDestroy()
		widget.tabPanel:destroy()
	end
