-- chunkname: @/modules/corelib/ui/uicombobox.lua
...
function UIComboBox:addOption(text, data, short, icon)
...
function UIComboBox:onMousePress(mousePos, mouseButton)
	...
	menu:display({
		x = self:getX(),
		y = self:getY() + self:getHeight() + (self.isGame and 0 or 2)
	})
	connect(menu, {
		onDestroy = function()
			self:setOn(false)
...
function UIComboBox:onMouseWheel(mousePos, direction)
	if not self.mouseScroll or self.disableScroll or not g_keyboard.isShiftPressed() then
		return false
	end
