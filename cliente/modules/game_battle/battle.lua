...
function init()
	...
	battleButton = modules.client_topmenu.addRightButton("battleButton", tr("Battle") .. " (Ctrl+B)", "/images/topbuttons/icon_battle", toggle, false, 6)
	...
	battleWindow:setContentMinimumHeight(95)
...
function checkCreatures()
	...
	local spectators = g_map.getSpectatorsInRangeEx(position, false, math.floor(dimension.width / 2), math.floor(dimension.width / 2), math.floor(dimension.height / 2), math.floor(dimension.height / 2))
	...
	for _, creature in ipairs(spectators) do
		if doCreatureFitFilters(creature) and maxCreatures > #creatures then
			if not creature.lastSeen or resetAgePoint > creature.lastSeen then
...
function doCreatureFitFilters(creature)
	...
	if g_game.isPokemonView() and creature:isSummon() then
		return false
	end
	...
	if pos.z ~= position.z or not creature:canBeSeen() then
		return false
	end
	...
	local hidePokemons = filterPanel.buttons.hidePokemons:isChecked()

	if hidePlayers and creature:isPlayer() then
		return false
	elseif hideNPCs and creature:isNpc() then
		return false
	elseif hideMonsters and creature:isMonster() and not creature:isSummon() and not creature:isOtherSummon() then
		return false
	elseif hidePokemons and (creature:isSummon() or creature:isOtherSummon()) then
		return false
	end
...
function sortCreatures(creatures)
	...
	if getSortType() == "distance" then
		...
		table.sort(creatures, function(a, b)
			if getDistanceBetween(position, a:getPosition()) == getDistanceBetween(position, b:getPosition()) then
				return ages[a:getId()] > ages[b:getId()]
			end

			return getDistanceBetween(position, a:getPosition()) > getDistanceBetween(position, b:getPosition())
		end)
