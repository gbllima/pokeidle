...
exitWindow = nil
bottomSplitter = nil
useItemConfirmationWindow = nil
limitedZoom = false
hookedMenuOptions = {}
lastDirTime = g_clock.millis()

local visionModeDelay = 200
local lastVisionModeChange = g_clock.millis()

function init()
	g_ui.importStyle("styles/countwindow")
	g_ui.importStyle("styles/itemdropconfirmationwindow")
	connect(g_game, {
	...
	logoutButton = modules.client_topmenu.addLeftButton("logoutButton", tr("Exit"), "/images/topbuttons/icon_logout", tryLogout, false, 1)
...
function bindKeys()
	...
	g_keyboard.bindKeyDown("Ctrl+=", decreaseGameVision, gameRootPanel)
	g_keyboard.bindKeyDown("Ctrl+-", increaseGameVision, gameRootPanel)
	g_keyboard.bindKeyPress("Ctrl+Shift+=", function()
...
function onGameStart()
	...
	-- Iniciar o módulo PlayerBar aqui
	if modules.game_playerbar then
		modules.game_playerbar.init()
	end
end
...
function onGameEnd()
	...
	-- Ocultar PlayerBar no logout
	if modules.game_playerbar then
		modules.game_playerbar.offline()
	end
end
...
function onVisionModeChange(mode)
	gameMapPanel:setVisibleDimension(visionModes[g_settings.getNumber("viewMode")][mode])
	modules.client_options.setOption("visionMode", mode + 1)
end

function increaseGameVision()
	if lastVisionModeChange > g_clock.millis() then
		modules.game_textmessage.displayStatusMessage("Vision Mode\nYou are exhausted")

		return
	end

	lastVisionModeChange = g_clock.millis() + visionModeDelay

	local mode = g_map.getVisionMode() + 1
	local vision = mode > 2 and g_game.getLocalPlayer():getGroupId() < 4 and VisionMode.Big or mode

	if vision <= VisionMode.Last then
		g_map.setVisionMode(vision)
	end
end

function decreaseGameVision()
	if lastVisionModeChange > g_clock.millis() then
		modules.game_textmessage.displayStatusMessage("Vision Mode\nYou are exhausted")
	...
	local vision = g_map.getVisionMode() - 1

	if vision >= VisionMode.First then
		g_map.setVisionMode(vision)
	end
end
...
function hide()
	...
	if useItemConfirmationWindow then
		useItemConfirmationWindow:destroy()
...
function onMouseGrabberRelease(self, mousePosition, mouseButton)
	...
	if clickedWidget then
		if selectedType == "use" then
			onUseWith(clickedWidget, mousePosition, self.callback)
...
function onUseWith(clickedWidget, mousePosition, callback)
	...
	elseif clickedWidget:getClassName() == "UIItem" and not clickedWidget:isVirtual() then
		if callback then
			if selectedThing:canConfirmUse() then
				displayItemUseConfirmationWindow(selectedThing, tr("You really want to use this item?"), "useItem", function(item)
					callback(item, clickedWidget:getItem(), selectedSubtype)
				end)
			else
				callback(selectedThing, clickedWidget:getItem(), selectedSubtype)
			end
		elseif selectedThing:canConfirmUse() then
			displayItemUseConfirmationWindow(selectedThing, tr("You really want to use this item?"), "useItem", function(item)
				g_game.useWith(item, clickedWidget:getItem(), selectedSubtype)
			end)
		else
...
function startUseWith(thing, subType, callback)
	...
	mouseGrabberWidget.callback = callback
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
    ...
    shortcut = not classic and not g_app.isMobile() and "(Shift)" or nil
    ...
    shortcut = not classic and not g_app.isMobile() and "(Ctrl)" or nil
    ...
    if useThing then
        ...
        elseif useThing:isMultiUse() then
            ...
            menu:addOption(tr("Use"), function()
                if useThing:canConfirmUse() then
                    displayItemUseConfirmationWindow(useThing, tr("You really want to use this item?"), "useItem")
                else
                    g_game.use(useThing)
                end
            end, shortcut)
    ...
    if creatureThing then
        ...
        if creatureThing:isSummon() then
            menu:addSeparator()
            menu:addOption(tr("Set Custom"), modules.game_custompokemon.show)
            menu:addOption(tr("Evolutions"), modules.game_evolutions.requestOpen)
        end

        menu:addOption(tr("Offset"), function()
            local looktype = creatureThing:getOutfit().type
            if looktype and looktype > 0 then
                local offset = require('game_offset')
                offset.OffsetManager.openWithLooktype(looktype)
            end
        end)
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature, marking)
    ...
    elseif not modules.client_options.getOption('classicControl') then
        -- INTERAÇÃO COM NPCs - CONTROLE NÃO-CLÁSSICO
        if creatureThing and creatureThing:isNpc() then
            local player = g_game.getLocalPlayer()
            if Position.distanceBetween(creatureThing:getPosition(), player:getPosition()) <= 3 then
                g_game.talkChannel(MessageModes.NpcTo, MessageModes.NpcTo, 'hi')
                g_game.talkChannel(MessageModes.NpcTo, MessageModes.NpcTo, 'trade')
            else
                modules.game_textmessage.displayFailureMessage('Você está muito longe para conversar com esse NPC')
            end
            return true
        ...
        elseif useThing and keyboardModifiers == KeyboardCtrlModifier and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
            ...
            elseif useThing:isMultiUse() then
                startUseWith(useThing)
                return true
            elseif useThing:getId() == 23827 then
                modules.game_traininglevel.onShowGuideWindow()
            else
                if useThing:canConfirmUse() then
                    displayItemUseConfirmationWindow(useThing, tr("You really want to use this item?"), "useItem")
                else
    ...
    elseif useThing and keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
        ...
        -- INTERAÇÃO COM NPCs - CONTROLE CLÁSSICO
        if creatureThing and creatureThing:isNpc() then
            if Position.distanceBetween(creatureThing:getPosition(), player:getPosition()) <= 3 then
                g_game.talkChannel(MessageModes.NpcTo, MessageModes.NpcTo, 'hi')
                g_game.talkChannel(MessageModes.NpcTo, MessageModes.NpcTo, 'trade')
    ...
    if autoWalkPos and keyboardModifiers == KeyboardNoModifier and (mouseButton == MouseLeftButton or mouseButton == MouseTouch2 or mouseButton == MouseTouch3) then
        ...
        if modules.client_options.getOption("walkClick") then
...
function moveStackableItem(title, item, toPos, callback)
	if countWindow then
		return
	end
	...
	if g_keyboard.isCtrlPressed() then
		if callback then
			callback(item, toPos, item:getCount())
		else
	...
	local function moveFunc()
		...
		if callback then
			callback(item, toPos, itembox:getItemCount())
		else
...
function refreshViewMode()
	local viewMode = g_settings.getNumber("viewMode")
	...
	for i = 1, gameRightPanels:getChildCount() + gameLeftPanels:getChildCount() do
		...
		if viewMode == ViewMode.Classic then
	...
	if viewMode == ViewMode.Classic then
		...
		bottomSplitter:setWidth(bottomSplitter.extendedViewWidth)
	...
	if modules.game_actionbar then
		modules.game_actionbar.switchMode(viewMode == ViewMode.Extended)
	end
	...
	g_map.setVisionMode(tonumber(modules.client_options.getOption("visionMode") or 1) - 1)
	updateSize()
end
...
function updateSize()
	...
	local viewMode = classic and ViewMode.Classic or ViewMode.Extended
	...
	local visionMode = g_map.getVisionMode()

	g_map.setVisionMode(visionMode)
end
...
function displayItemDropConfirmationWindow(item, message, optionId, confirmCallback, cancelCallback)
	local window = g_ui.createWidget("DropItemConfirmationWindow", rootWidget)

	window:onVisibilityChange(true)
	window.item:setItem(item)
	window.messageLabel:setText(message)

	window.buttonConfirm.onClick = confirmCallback
	window.buttonCancel.onClick = cancelCallback

	local checkbox = window.checkbox

	checkbox:setId(optionId)
	checkbox:setChecked(not modules.client_options.getOption(optionId))
	connect(window, {
		onEnter = confirmCallback
	})
	connect(window, {
...
function displayItemUseConfirmationWindow(item, message, optionId, confirmCallback)
	...
	useItemConfirmationWindow = displayItemDropConfirmationWindow(item, message, optionId, defaultConfirm, defaultCancel)
end
