...
function UIGameMap:onDrop(widget, mousePos)
	...
	if thingPos and thingPos.x == toPos.x and thingPos.y == toPos.y and thingPos.z == toPos.z then
		return false
	end

	if thing:isItem() then
		if thing:getCount() > 1 then
			modules.game_interface.moveStackableItem("Move Itens", thing, toPos)

			return true
		elseif not self.currentDragThing then
			if #thing:getPokemon() > 0 and modules.client_options.getOption("dropPokeball") then
				if itemDropConfirmationWindow then
					itemDropConfirmationWindow:destroy()
				end
				...
				local function confirmCallback()
					...
					itemDropConfirmationWindow = nil
				end
				...
				itemDropConfirmationWindow = displayItemDropConfirmationWindow(thing, tr("You really want to drop that pokeball?"), "dropPokeball", confirmCallback, cancelCallback)

				return true
			elseif thing:isContainer() and modules.client_options.getOption("dropContainer") then
				if itemDropConfirmationWindow then
					itemDropConfirmationWindow:destroy()
				end
				...
				itemDropConfirmationWindow = displayItemDropConfirmationWindow(thing, tr("You really want to drop that container?"), "dropContainer", confirmCallback, cancelCallback)
