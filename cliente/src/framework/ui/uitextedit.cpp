...
static std::string extractTr(const std::string &v)
{
    std::string s = v;
    size_t start = 0;
    ...
    if (s.rfind("tr(", 0) == 0 && s.size() >= 4 && s.back() == ')')
    {
...
void UITextEdit::drawSelf(Fw::DrawPane drawPane)
{
    ...
    if (textLength == 0)
    {
        UIWidgetPtr ph = getChildById("placeholderLabel");
        if ((!ph || !ph->isVisible()) && m_placeholderColor != Color::alpha && !m_placeholder.empty())
        {
...
void UITextEdit::update(bool focusCursor)
{
    ...
    if (focusCursor && m_autoScroll)
    {
        ...
        {
            VALIDATE(m_cursorPos <= textLength);
            Rect virtualRect(m_textVirtualOffset, m_rect.size() - Size(m_padding.left + m_padding.right, 0));
    ...
    else
    {
        if (m_cursorPos > 0 && textLength > 0)
        {
            Rect virtualRect(m_textVirtualOffset, m_rect.size() - Size(2 * m_padding.left + m_padding.right, 0));
            int pos = m_cursorPos - 1;
            glyph = (uchar)text[pos];
            Rect glyphRect(glyphsPositions[pos], glyphsSize[glyph]);
            if (virtualRect.contains(glyphRect.topLeft()) && virtualRect.contains(glyphRect.bottomRight()))
    ...
    for (int i = 0; i < textLength; ++i)
    {
        int glyphIndex = (uchar)text[i];
        m_glyphsCoords[i].clear();
        if (glyphIndex < 32)
            continue;

        Rect glyphScreenCoords(glyphsPositions[i], glyphsSize[glyphIndex]);
        Rect glyphTextureCoords = glyphsTextureCoords[glyphIndex];
...
void UITextEdit::onStyleApply(const std::string &styleName, const OTMLNodePtr &styleNode)
{
    ...
    for (const OTMLNodePtr &node : styleNode->children())
    {
        ...
        else if (node->tag() == "placeholder" || node->tag() == "!placeholder")
            setPlaceholder(extractTr(node->value()));
...
bool UITextEdit::onKeyPress(uchar keyCode, int keyboardModifiers, int autoRepeatTicks)
{
    ...
    {
        if (keyCode == Fw::KeyDelete && m_editable)
        {
        ...
        else if (keyCode == Fw::KeyBackspace && m_editable)
        {
            if (hasSelection() || !m_text.empty())
            {
                del(false);
        ...
        else if (keyCode == Fw::KeyRight && !m_shiftNavigation)
        {
            clearSelection();
            moveCursorHorizontally(true);
            return true;
        }
        else if (keyCode == Fw::KeyLeft && !m_shiftNavigation)
        {
            clearSelection();
